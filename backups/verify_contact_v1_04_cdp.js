...
async function main() {
  ...
  for (const [width,height] of [[390,844],[393,873],[412,915]]) {
    ...
    fs.writeFileSync(`backups/contact_v1_04_${width}x${height}_geometry.json`,JSON.stringify(data,null,2));
    const shot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false,fromSurface:true});
    fs.writeFileSync(`backups/contact_v1_04_${width}x${height}.png`,Buffer.from(shot.data,'base64'));
